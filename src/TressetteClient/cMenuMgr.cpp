...
static const char* lpszVersion = "Ver 2.2.0 20190801"; // formato simile a 'Ver 0.5.4 14042008'
...
*/
void cMenuMgr::drawBackground(BOOL isOption)
{
    ...
    // header bar
    if (isOption) {
        fillRect(m_iDebx, m_iDeby - 2, m_iSx - m_iDebx * 2, 38, c_bluefg);
    }
    else {
...
void   cMenuMgr::HandleOptionMenu()
{
    ...
    // Draw title bar
    drawStringSH(m_pLanString->GetStringId(cLanguages::ID_MEN_OPTIONS).c_str(), m_iDebx + 10, m_iDeby + 5, c, m_pfont1);
    ...
    drawStringSH(m_pLanString->GetStringId(cLanguages::ID_OPT_GENERAL).c_str(), m_iDebx + 10, m_iDeby + 50, c, m_pfont1);
    // Deck
    if (m_ifocus_valuesM_A != 1)
    {
        c = staColor_off;
    ...
    drawStringSH(m_pLanString->GetStringId(cLanguages::ID_MAZZONAME).c_str(), m_iDebx + 10, m_iDeby + 90, c, m_pfont1);
    // Match
    if (m_ifocus_valuesM_A != 2)
    {
        c = staColor_off;
